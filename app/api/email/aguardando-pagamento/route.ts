...
import { enviarEmailAguardandoPagamento } from '@/lib/email';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Enviar email
    const result = await enviarEmailAguardandoPagamento({
      nomeCliente,
      email,
      transactionId,
      items,
    ...
    return NextResponse.json({ 
      success: true,
      message: 'Email de aguardando pagamento enviado com sucesso',
      data: result.data
    });
  } catch (error) {
